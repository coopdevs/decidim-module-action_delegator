...
module Decidim
  module ActionDelegator
    describe ImportDelegationsMailer, type: :mailer do
      ...
      describe "#import" do
        context "when the CSV has valid rows" do
          ...
          it "renders the headers" do
            expect(mail.subject).to eq("Delegations imported")
        ...
        context "when the CSV has invalid rows" do
          ...
          it "attaches CSV file if file has invalid rows" do
            expect(mail.attachments["delegations_details.csv"]).not_to be_nil
          end
        end
      end
    end
